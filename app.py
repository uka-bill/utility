...
def initialize_database_tables():
    """Check if required tables exist - just check, don't create"""
    try:
        if not supabase:
            return
        ...
        try:
            supabase.table("financial_years").select("id").limit(1).execute()
            print("✅ Financial years table exists")
        except Exception as e:
            print("❌ Financial years table not found or error accessing it")
            print("⚠️ Please create the 'financial_years' table manually in Supabase")
            
        try:
            supabase.table("schools").select("id").limit(1).execute()
            print("✅ Schools table exists")
        except Exception as e:
            print("❌ Schools table not found or error accessing it")
            print("⚠️ Please create the 'schools' table manually in Supabase")
            
        try:
            supabase.table("departments").select("id").limit(1).execute()
            print("✅ Departments table exists")
        except Exception as e:
            print("❌ Departments table not found or error accessing it")
            print("⚠️ Please create the 'departments' table manually in Supabase")
            
        try:
            supabase.table("utility_bills").select("id").limit(1).execute()
            print("✅ Utility bills table exists")
        except Exception as e:
            print("❌ Utility bills table not found or error accessing it")
            print("⚠️ Please create the 'utility_bills' table manually in Supabase")
