...
def initialize_database_tables():
    ...
    try:
        ...
        try:
            ...
            print("❌ Users table not found or error accessing it")
            print("⚠️ Please create the 'users' table manually in Supabase")
            
        try:
            supabase.table("password_resets").select("id").limit(1).execute()
            print("✅ Password resets table exists")
        except Exception as e:
            print("❌ Password_resets table not found or error accessing it")
            print("⚠️ Please create the 'password_resets' table manually in Supabase")
    ...
    if request.method == 'POST':
        try:
            ...
            if app.config['MAIL_USERNAME'] and app.config['MAIL_PASSWORD']:
                ...
                # Email not configured, return token for testing
                return jsonify({
                    'success': True, 
                    'message': 'Password reset token created (email not configured).',
                    'reset_token': reset_token,
                    'reset_link': f"{request.host_url}reset-password?token={reset_token}"
                })
