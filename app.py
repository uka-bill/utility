...
def create_school():
    ...
    try:
        ...
        school_data = {
            ...
            "address": data.get('address', ''),
            "notes": data.get('notes', ''),
            # NEW FIELDS for utility accounts
            "water_account": data.get('waterAccount', ''),
            "water_meter": data.get('waterMeter', ''),
            "electricity_account": data.get('electricityAccount', ''),
            "electricity_meter": data.get('electricityMeter', ''),
...
def create_utility_bill():
    ...
    try:
        ...
        # Get entity name and default account info
        entity_name = ""
        water_account = ""
        water_meter = ""
        electricity_account = ""
        ...
        if data.get('entity_type') == 'school':
            school_response = supabase.table("schools").select("name, water_account, water_meter, electricity_account, electricity_meter, telephone_account, telephone_number").eq("id", data.get('entity_id')).execute()
            if school_response.data and len(school_response.data) > 0:
                entity_name = school_response.data[0]['name']
                water_account = school_response.data[0].get('water_account', '')
                water_meter = school_response.data[0].get('water_meter', '')
                electricity_account = school_response.data[0].get('electricity_account', '')
                electricity_meter = school_response.data[0].get('electricity_meter', '')
                telephone_account = school_response.data[0].get('telephone_account', '')
                telephone_number = school_response.data[0].get('telephone_number', '')
        elif data.get('entity_type') == 'department':
            dept_response = supabase.table("departments").select("name, water_account, water_meter, electricity_account, electricity_meter, telephone_account, telephone_number").eq("id", data.get('entity_id')).execute()
            if dept_response.data and len(dept_response.data) > 0:
                entity_name = dept_response.data[0]['name']
                water_account = dept_response.data[0].get('water_account', '')
                water_meter = dept_response.data[0].get('water_meter', '')
                electricity_account = dept_response.data[0].get('electricity_account', '')
                electricity_meter = dept_response.data[0].get('electricity_meter', '')
                telephone_account = dept_response.data[0].get('telephone_account', '')
                telephone_number = dept_response.data[0].get('telephone_number', '')
        
        # Get current date for default values
        current_date = datetime.now()
        
        # If account number is not provided in request, use the one from entity
        account_number = data.get('account_number')
        if not account_number:
        ...
        # If meter number is not provided, use the one from entity
        meter_number = data.get('meter_number')
        if not meter_number and data.get('utility_type') == 'water':
            meter_number = water_meter
        elif not meter_number and data.get('utility_type') == 'electricity':
            meter_number = electricity_meter
        
        # If phone number is not provided, use the one from entity
        phone_number = data.get('phone_number')
        if not phone_number and data.get('utility_type') == 'telephone':
...
@app.route('/api/entity-details')
def api_entity_details():
    """Get detailed information for an entity including utility accounts"""
    try:
        entity_type = request.args.get('entity_type')
        entity_id = request.args.get('entity_id')
...
def api_entity_accounts():
    ...
    try:
        ...
        # First, try to get from the entity table (schools or departments)
        accounts = []
        ...
        if entity_type == 'school':
            ...
            if response.data and len(response.data) > 0:
                school = response.data[0]
                if utility_type == 'water' and school.get('water_account'):
                    accounts.append(school.get('water_account'))
                elif utility_type == 'electricity' and school.get('electricity_account'):
                    accounts.append(school.get('electricity_account'))
                elif utility_type == 'telephone' and school.get('telephone_account'):
                    accounts.append(school.get('telephone_account'))
            ...
            if response.data and len(response.data) > 0:
                dept = response.data[0]
                if utility_type == 'water' and dept.get('water_account'):
                    accounts.append(dept.get('water_account'))
                elif utility_type == 'electricity' and dept.get('electricity_account'):
                    accounts.append(dept.get('electricity_account'))
                elif utility_type == 'telephone' and dept.get('telephone_account'):
                    accounts.append(dept.get('telephone_account'))
        
        # Also get from utility_bills table (historical accounts)
        bills_response = supabase.table("utility_bills").select("account_number").eq("entity_type", entity_type).eq("entity_id", entity_id).eq("utility_type", utility_type).execute()
        
        for bill in bills_response.data:
            if bill['account_number'] and bill['account_number'] not in accounts:
                accounts.append(bill['account_number'])
...
@app.route('/api/entity-meters')
def api_entity_meters():
    """Get meter numbers for an entity (for water/electricity)"""
    try:
        entity_type = request.args.get('entity_type')
        entity_id = request.args.get('entity_id')
        utility_type = request.args.get('utility_type')
        ...
        # Try to get from the entity table (schools or departments)
        meters = []
        ...
        if utility_type in ['water', 'electricity']:
            if entity_type == 'school':
                ...
                if response.data and len(response.data) > 0:
                    school = response.data[0]
                    if utility_type == 'water' and school.get('water_meter'):
                        meters.append(school.get('water_meter'))
                    elif utility_type == 'electricity' and school.get('electricity_meter'):
                        meters.append(school.get('electricity_meter'))
            
            elif entity_type == 'department':
                response = supabase.table("departments").select("*").eq("id", int(entity_id)).execute()
                if response.data and len(response.data) > 0:
                    dept = response.data[0]
                    if utility_type == 'water' and dept.get('water_meter'):
                        meters.append(dept.get('water_meter'))
                    elif utility_type == 'electricity' and dept.get('electricity_meter'):
                        meters.append(dept.get('electricity_meter'))
        
        # Also get from utility_bills table (historical meters)
        field = 'meter_number'
        bills_response = supabase.table("utility_bills").select(field).eq("entity_type", entity_type).eq("entity_id", entity_id).eq("utility_type", utility_type).execute()
        
        for bill in bills_response.data:
            if bill.get(field) and bill.get(field) not in meters:
                meters.append(bill.get(field))
        
        return jsonify(meters)
        
    except Exception as e:
        print(f"❌ Entity meters error: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/entity-phone-numbers')
def api_entity_phone_numbers():
    """Get telephone numbers for an entity"""
    try:
        entity_type = request.args.get('entity_type')
        entity_id = request.args.get('entity_id')
        ...
        if entity_type == 'school':
            ...
            if response.data and len(response.data) > 0:
                school = response.data[0]
                if school.get('telephone_number'):
                    phone_numbers.append(school.get('telephone_number'))
        
        elif entity_type == 'department':
            response = supabase.table("departments").select("*").eq("id", int(entity_id)).execute()
            if response.data and len(response.data) > 0:
                dept = response.data[0]
                if dept.get('telephone_number'):
                    phone_numbers.append(dept.get('telephone_number'))
        
        # Also get from utility_bills table (historical phone numbers)
        bills_response = supabase.table("utility_bills").select("phone_number").eq("entity_type", entity_type).eq("entity_id", entity_id).eq("utility_type", "telephone").execute()
        
        for bill in bills_response.data:
            if bill.get('phone_number') and bill.get('phone_number') not in phone_numbers:
                phone_numbers.append(bill.get('phone_number'))
        
        return jsonify(phone_numbers)
        
    except Exception as e:
        print(f"❌ Entity phone numbers error: {e}")
        return jsonify({'error': str(e)}), 500
